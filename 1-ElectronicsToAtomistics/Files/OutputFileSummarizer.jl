...
function gsfe(structure, lp, slip_system=nothing)
    ...
    filenum, energy = 1, []; for d in fault_points
...
function run_qe(filenum)
    ...
    # get energy
    io = IOBuffer()
    cmd_sed_totalenergy_line    = `grep '! *[ ] total energy' gsfe_$filenum.out`
    cmd_sed_totalenergy          = `awk '{print $5}'`
    # pipeline_cmd1 = run(Cmd(["$cmd1", " | ", "$cmd2"]))
    cmd_pl_totalenergy      = run(pipeline(cmd_sed_totalenergy; stdin=cmd_sed_totalenergy_line, stdout=io))
    ...
    totalenergy_str         = try
        ...
        if isa(exc, TypeError)
            io = IOBuffer()
            cmd_sed_totalenergy_line    = `sed -n 's%\!*[[:space:]]*total energy[[:space:]]* = [[:space:]]*%%p' gsfe_$filenum.out`
            cmd_sed_totalenergy         = `sed 's% Ry$%%'`
            cmd_tail_lastline           = `tail -n1`
            cmd_pl_totalenergy          = run(pipeline(cmd_tail_lastline; stdin=pipeline(cmd_sed_totalenergy; stdin=cmd_sed_totalenergy_line), stdout=io))
            String(take!(io))
        end
    end
...
function run_vasp()
    ...
    while tries < 2 && !has_run
        try
            ...
            # create the command for tail
            cmd1 = `tail -n1 OSZICAR`
            # create the command for awk
            cmd2 = `awk '{print $5}'`
            # chain the commands using a pipeline
