...
def write_qe_inputs(lp, basis, layers, atoms):
	...
	with open('gsfe.in','w') as f:
		...
		f.write("mixing_mode ='local-TF', electron_maxstep = 250," + os.linesep)
		f.write("mixing_beta = 0.575, conv_thr = 0.000001," + os.linesep)
		f.write(' / ' + os.linesep)
		...
		f.write(' {0} {0} 2 0 0 0 '.format(kpoints) + os.linesep)
		# Basis vectors
...
def gsfe( struct, lp, slip_system=None ):
	...
	for d in fault_points:
		...
		os.system('cp gsfe.in ./input_gens/gsfe_{}.in'.format(filecount))
		with open('RE_comm.cm','a') as f:
			f.write("mpirun -np 16 pw.x -i gsfe_{}.in > gsfe_{}.out".format(filecount,filecount)+ os.linesep)
